...
#!/usr/bin/env python3
"""
companion_web_v10e.py
- Structured Edit Mode (BACKGROUND / FACE / BODY / HANDS / WARDROBE / CAMERA+LIGHT / STYLE)
- Scene Composer (env kits, style packs, creative boost, variation)
- Heartbeat polling to keep last frame visible (no grey gaps)
- Input source selector (Base / Last render / Custom) + optional upload of last render to Comfy input
- LLM backends: OpenAI or Ollama (default: Ollama llama3.1:8b)
- Memory integration (human-readable YAML via memory_manager.py)
  * Append transcript + gestures each turn
  * Buttons: New Character, Save Memory, Summarize Session, Pin Fact
  * Memory slice injected into system prompt
- Album integration (album_manager.py)
  * Button: Save to Album
  * Gallery: thumbnail grid with prompt captions
"""

import os, json, glob, subprocess, datetime, time, re
import requests
import gradio as gr
...
MODEL_OLLAMA_DEFAULT = "llama3.1:8b"  # good local default
...
def compose_structured_prompt(sections, template, use_background, use_body, use_face, use_hands, use_wardrobe, use_camera, use_style,
                              s_bg, s_body, s_face, s_hands, s_wardrobe, s_camera, s_style):
...
def plan_step(chat_messages, json_history, seed,
              ...
              use_bg, use_body, use_face, use_hands, use_wardrobe, use_cam, use_style,
              s_bg, s_body, s_face, s_hands, s_wardrobe, s_cam, s_style,
    ...
    # Memory: append assistant + gesture
    if mem is not None and mm is not None:
        try:
    ...
    if structured_mode:
        ...
        visual = ", ".join([sections["BACKGROUND"], sections["FACE"], sections["BODY"], sections["HANDS"], sections["WARDROBE"], sections["CAMERA_LIGHT"], sections["STYLE"]])
...
def summarize_session(mem, mem_path, transcript_tail_json, provider, model):
    # Use LLM to generate a short episode summary from recent transcript tail
    if mem is None or mm is None:
        return "Memory unavailable."
    try:
        tail = json.loads(transcript_tail_json) if transcript_tail_json else []
    except Exception:
        tail = []
    text = "\n".join(f"{m.get('role','user')}: {m.get('content','')}" for m in tail[-30:])
    sys = "Write a concise 2–3 sentence summary of the conversation, focusing on decisions, emotions, and scene changes."
    ...
    try:
        ...
        # If the backend forced JSON, resp may be dict; accept string fallback
        summary = resp if isinstance(resp, str) else (resp.get("reply") or resp.get("summary") or "Summary.")
    except Exception:
        # fallback trivial summary
        summary = "Companion and user explored scenes and emotions; visual edits were composed and rendered."
    try:
...
# ---------- UI ----------
def build_app():
    with gr.Blocks(title="Companion v10e — Memory + Album") as demo:
        gr.Markdown("## Companion v10e — Memory + Album + Structured Edit")
        ...
        with gr.Row():
            ...
            with gr.Column(scale=1):
                ...
                txt = gr.Textbox(placeholder="Scene or change (e.g., 'gothic cathedral, candle in hand, dark coat')", show_label=False)
                ...
                with gr.Accordion("Character & LLM", open=True):
                    ...
                    provider = gr.Dropdown(choices=["ollama","openai","none"], value="ollama", label="LLM provider")
                    model = gr.Textbox(value=MODEL_OLLAMA_DEFAULT, label="Model")
                ...
                with gr.Accordion("Structured Edit Mode", open=True):
                    ...
                    template_text = gr.Textbox(value=DEFAULT_TEMPLATE, lines=12, label="Prompt template (editable)")
        ...
        save_mem_btn.click(
            save_memory,
            inputs=[mem_state, mem_path_state],
            outputs=[llmstatus]
        )
        ...
        pin_btn.click(
            pin_fact,
            inputs=[mem_state, mem_path_state, pin_text],
            outputs=[llmstatus]
        )
        ...
        summarize_btn.click(
            summarize_session,
            inputs=[mem_state, mem_path_state, chat_state, provider, model],
            outputs=[llmstatus]
        )
        ...
        # Step 0: initialize memory on app load with default name
        ...
        chain = chain.then(
            ...
            inputs=[chat_state, json_state, seed,
                    input_src_mode, base_image_path, custom_image_path,
                    workflow, outputs_dir,
                    gr.Textbox(value=DEFAULT_SYSTEM_PROMPT_BASE, visible=False),  # placeholder system prompt
